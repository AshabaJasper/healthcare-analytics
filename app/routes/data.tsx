...
export const meta: MetaFunction = () => {
  return [
    ...
    { name: "description", content: "View and filter healthcare data" },
  ];
};
...
export async function loader({ request }: LoaderFunctionArgs) {
  try {
    ...
    console.error("Error in data route loader:", error);
...
export default function Data() {
  // Explicitly define the loader return type to avoid type issues
  type LoaderData = {
    records: any[];
    pagination: {
      page: number;
  ...
  const { records, pagination, filterOptions } = useLoaderData<LoaderData>();
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="mt-2 text-gray-600">
              View and filter your healthcare data. Use the search and filters to narrow
              down the results.
            </p>
          </div>
          <div className="text-right">
            <p className="text-sm text-gray-600">
              Showing {Math.min(filteredData.length, pagination.pageSize)} of {pagination.total} records
            </p>
          </div>
        </div>
          ...
          <div className="p-6">
            {records && records.length > 0 ? (
              ...
              <div className="text-center py-10">
                <p className="text-gray-500">No records found. Adjust filters or upload data to begin.</p>
              </div>
            )}
          </div>
        </div>
            ...
            <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
              <div>
                <p className="text-sm text-gray-700">
                  Showing <span className="font-medium">{pagination.total > 0 ? (currentPage - 1) * currentPageSize + 1 : 0}</span> to{" "}
